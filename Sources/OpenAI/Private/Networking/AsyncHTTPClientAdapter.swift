...
public class AsyncHTTPClientAdapter: HTTPClient {
    ...
    public func data(for request: HTTPRequest) async throws -> (Data, HTTPResponse) {
        let ahcRequest = try createAHCRequest(from: request)

        let response = try await client.execute(ahcRequest, deadline: .now() + .seconds(60))
    ...
    private func createAHCRequest(from request: HTTPRequest) throws -> HTTPClientRequest {
        var ahcRequest = HTTPClientRequest(url: request.url.absoluteString)
		ahcRequest.method = NIOHTTP1.HTTPMethod(rawValue: request.method.rawValue) // Fix this

        // Add headers
        for (key, value) in request.headers {
            ahcRequest.headers.add(name: key, value: value)
        }
        
        // Add body if present
        if let body = request.body {
            ahcRequest.body = .bytes(body)
        }
